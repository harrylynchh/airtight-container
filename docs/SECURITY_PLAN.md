...
- ✅ `cap_drop: [ALL]`, `security_opt: [no-new-privileges:true]`, `pids_limit`, `mem_limit` (predated this) on both services. **✅ 2026-06-20 Docker-validated:** built both images locally, ran each workload under these exact constraints — Chromium rendered a valid PDF as non-root `app`, nginx-unprivileged served the SPA (uid 101), and the Node backend booted to `"server listening"`. No restarts/OOM/cap errors.
