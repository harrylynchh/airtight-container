...
## TL;DR — audit sweep MERGED BUT NOT DEPLOYED (CI fix in PR #32); 2026-09-03 Places autofill outage RESOLVED
...
**Audit sweep — PR #31, MERGED 2026-09-05 (`main` = `7b72250`) but NOT DEPLOYED.** 28 files, +1061/−170. The merge triggered `Deploy` on `main`; its new `test` job failed, and because `deploy` now `needs: test`, **the deploy never ran — prod is still on the pre-sweep image.** None of the 20 fixes are live.

**Why it failed, and the fix — PR #32 `fix/ci-fresh-db-baseline` (open).** The `test` job assumed `scripts/migrate.ts` could build a DB from nothing. It cannot: `0000_phase1_additive` **ALTERs** `inventory`, `sold`, `release_numbers` — v1 tables that predate the runner and that no migration creates. Empty DB → `relation "release_numbers" does not exist`. Fix adds `server/db/ci-baseline.sql` (schema-only dump at 0024) and bootstraps: `node migrate.js` → `psql -f db/ci-baseline.sql` → `migrate.ts --baseline-on-empty 0024`. Newer migrations still get applied and exercised; only the historical chain is skipped. **Rehearsed against a truly empty DB this time** (the sandbox no longer blocks `CREATE DATABASE`): resulting schema is byte-identical to the dev DB per `pg_dump` diff, suite 265/265 against it. PR #32 also adds a `pull_request` trigger so the suite gates PRs instead of first running post-merge, with `deploy` gated `if: github.event_name == 'push'`.

**MERGING #32 IS WHAT SHIPS THE SWEEP** — it lands on `main`, `test` passes, `deploy` finally runs and pushes both images to EC2. Expect the first real deploy of all 20 fixes at that moment; watch it.
...
**P&L CHANGE MERGED WITHOUT EXPLICIT SIGN-OFF — owner should be aware:** the P&L fix makes `revenue` exclude the trucking pass-through (`report-resolvers/pnl.ts:234`). It matches the report's own "Trucking pass-through (not in profit)" row label and makes profit correct, but the "Sales Revenue" headline in `PnLTemplate.tsx:47` will read **lower** by the trucking total once deployed, and old P&L PDFs won't reconcile against new ones. It is in `main` and will go live with the next deploy. Revert it before merging #32 if that headline shift is not wanted.

**Deliberate residual gaps (not blockers):** `createdInvoice` in `CreateInvoice.tsx` is in-memory only — a full browser reload mid-error still mints a new invoice number; closing it means adding the field to `draftSnapshot`. (The CI fresh-DB bootstrap gap is now closed by PR #32, above.) HTTP-level tests for the `/company` endpoints in `release.js`/`pickup.js` were blocked by a vitest/Vite resolution quirk (`middleware/validate.js` is `.ts`-only and fails to resolve once the better-auth chain is mocked); a DB-level constraint test was substituted.
