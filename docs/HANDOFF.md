...
- Public `/sms-terms` page (`client/src/routes/SmsTerms.tsx`) renders the same disclosure for Twilio campaign reviewers to verify without logging in. Wired through `App.jsx` route table + `userContext.jsx` auth-redirect exclusion. Navbar hidden on that path.
