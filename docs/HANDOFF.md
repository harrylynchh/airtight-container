...
- **Twilio A2P 10DLC campaign** — submit. Messaging policy URL = `https://airtightshippingcontainer.com/sms-terms`, privacy policy URL = `https://airtightshippingcontainer.com/privacy-policy`. Both public, both verified post-cutover. Add Twilio creds (`TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_MESSAGING_SERVICE_SID=MGde4ad37ed70fb2bd1bd9330c009ced23`) to EC2 `~/airtight-container/.env` before the campaign goes live; SMS Send returns 503 until then.
