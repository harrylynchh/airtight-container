...
### Operator-meeting feedback (cont.) — captured 2026-05-27

Overarching theme: **delivery sheets stop being standalone reports and become extensions of an invoice.** The invoice collects the delivery info (optional fields) once and carries it through to the sheet. Box selection is scoped by the invoice instead of picked from a flat list of all sold boxes.

- **Delivery sheets move from Reports → a per-invoice "Make Delivery Sheet" button.** On `/invoices/:id`, replace/sit alongside the current Regenerate-PDF / Edit / Delete button cluster with a **"Make Delivery Sheet"** action. It scopes the box choice to *only the containers on that invoice* (no more flat all-sold-boxes picker) and pre-fills the sheet from invoice-collected data. **Nuke the `delivery_sheet` report-menu option entirely** — the old per-box-number selection is deprecated; making a sheet "for a client (via their invoice)" is more intuitive than "for a box." Scope is tightened by the invoice, so the old customer-name search / box-selection step is moot.
- **Per-box delivery address + door orientation become persisted invoice data.** Add columns on `sold` (per-container) for delivery address + door orientation so the delivery-sheet button reads them back instead of re-asking. Door orientation is a dropdown — **"Doors to Cab" / "Doors to Rear" / write-in** — replacing the current freetext example string in `delivery.ts`.
- **Three-level address cascade.** (1) **Billing** = client's address on file. (2) **Invoice shipping** defaults to billing, with a checkbox to enter a different ship-to. (3) **Per-container delivery** defaults to the invoice shipping address (so if the operator overrode ship-to, the per-box default follows it), and is overridable per box. Worth breaking the per-container delivery-address collection into **its own Stepper step** in the invoice create flow (`client/src/routes/CreateInvoice.tsx`).
- **"Delivery To" stays a priced line item.** Pricing is unchanged — driven by the trucking rate entered at invoice creation, as today. What changes is the *content*: the line now renders the full delivery address + door orientation (when provided), not just a bare trucking charge.
- **Trucking companies become a first-class entity.** New table storing company name + business/dispatch contact (the contact *is* dispatch). Everything trucking-related FKs to it as appropriate — outbound trucker on the invoice/`sold`, `delivery_company` on the delivery sheet, and inbound `inventory.trucking_company` where it fits. Replaces the scattered freetext trucker fields.
- **Autofill delivery-sheet fields from client data where possible, all overridable.** In particular, **onsite contact defaults to the client name + phone**. General rule: pull client data into the requisite delivery fields as defaults the operator can override.
- **Delivery-sheet UID `ATYYYYMM###`.** e.g. `AT202605001`. The `###` is a monthly sequence that resets to `001` on the 1st (zero-padded to 3; overflow past 999 not a concern at this volume). **Persist** the UID on the record so reprints are stable. Delivery sheets only for now — not other report types.
- **Real outbound flow — BUILT 2026-05-27 on `phase-struct/structural-batch` (awaiting operator test; migration 0016 not yet on prod).** Shipped: `ATYYYYMM###` numbering (migration 0016 + advisory-lock sequencer), `POST /report/:id/complete-pickup` (receipt-print = outbound, auto-flip cron kept as fallback), `/outbound` search/confirm screen + `GET /report/by-number/:number`, and removal of outbound-date from the invoice flow. Verified end-to-end on the local mirror. Original spec below for reference. Two distinct events: a delivery sheet is created when trucking is **scheduled**; the receipt is printed when pickup is **complete**, and *receipt-print is the outbound event* (`sold.outbound_date` = receipt render time). UX: a simple outbound screen searchable by the **AT delivery-sheet number** → driver enters SMS info (if applicable) → confirm delivery details → print receipt (which stamps outbound). **Decisions:** (1) receipt-print is the *primary* trigger, but **keep the date-based auto-flip** (`applyOutboundFromDeliverySheets()`, daily `OUTBOUND_FLIP_CRON`) as a **fallback** for sheets where no receipt ever gets printed — don't remove it, just stop treating it as the source of truth. (2) **Add the `ATYYYYMM###` delivery-sheet numbering now** (persisted on the `reports` row, monthly reset, sequenced like invoice numbers via advisory lock) so the outbound screen can search by it. Build pieces: AT-number column + sequencer on delivery-sheet reports; a receipt-print endpoint that flips `state → outbound` + stamps `sold.outbound_date = now`; an outbound search/confirm screen; driver SMS hook (gated on Twilio, see HANDOFF). Rides on the *current* report-based delivery sheets; the delivery-sheet-as-invoice-extension redesign can come later.
- **Receipt fixes.** (1) **Timestamp on render** — stamp the actual print time on the receipt; this *is* the outbound date (see above). (2) **Signature layout** — fix the signature block rolling onto a second page (CSS/layout only, `DeliveryReceiptTemplate.tsx`).
- **Printer from a Windows device (Q raised, decision deferred).** `window.print()` already works from Windows — it opens the standard Windows print dialog, which can target any installed printer (incl. a networked thermal printer); no code change for dialog-based printing. Silent / named-printer printing (zero-click) would need a small local print agent or browser kiosk-mode flag. Decide which UX we want when Phase 7 print work lands.
- **Clean container deletion in-app (available-only, reopens the release-# slot). — DONE 2026-05-27.** Replaced the unsafe `DELETE /api/v1/inventory/:id` (which never reset `release_number_containers.is_used`, stranding the slot) with a transactional route gated to `state = 'available'`: refuses non-available (409 `not_deletable_state`), refuses rows carrying invoice/sold history rather than cascading it away (409 `has_sale_history`), and flips the enumeration slot back to `is_used = false` so the unit number can be re-entered. UI: red "Delete container" button in `InventoryEditor` footer, available-only, two-step confirm, drops the row from the list on success. `release_number_count` is intentionally left unchanged (it's a quota, not a live count). Files: `server/routes/v1/inventory.js`, `client/src/components/forms/InventoryEditor.tsx`, `client/src/routes/Inventory.tsx`.
- **Inventory cleanup migration from the 2026-05-27 physical audit — see [docs/AUDIT_MIGRATION.md](AUDIT_MIGRATION.md).** The audit (ground truth for what's onsite) reconciled against prod surfaced dirty `unit_number` data (embedded spaces, dropped/typo'd check digits, duplicates with no unique constraint, test/junk rows) and 14 S&H boxes misfiled as sales stock. Planned destructive migration: normalize + dedupe unit numbers, delete the audited-as-S&H rows (operator re-adds by hand as `sh_inventory`), outbound-sweep everything sold/stale not in the audit, add a unique constraint. Blocked on a handful of operator decisions (see that doc's "Open decisions"). Read-only scripts: `server/scripts/audit-reconcile.sql`, `audit-cleanup-investigate.sql`.
- **S&H billing model needs two new shapes (from the 2026-05-27 physical audit).** Today `sh_inventory` only models in/out/daily (`in_fee`, `out_fee`, `daily_rate`). The audit surfaced two clients that don't fit: (1) **$0 free storage** — the Times Square "stock for our modifications" boxes should be *tracked as S&H but never invoiced* (zero-rate, or an explicit `non_billable` flag so they're excluded from month-end without polluting revenue). (2) **Flat per-unit monthly rate** — the Times Square `$325/u` boxes bill a flat monthly charge, not in/out/daily. Decide whether to add a per-box billing-mode (`in_out_daily` | `flat_monthly` | `free`) with a `flat_rate` column, and teach `sh-month-end.ts` to emit a flat line instead of in/out/storage lines for those. Affects the `sh_line_type` enum too (needs a `flat_month` line type).
- **S&H month-end cron — checked 2026-05-27, not active in prod (fine for now).** The `0 1 1 * *` `node-cron` job (`server.js:94`) is on `main` and defaults ON (`SH_MONTH_END_CRON !== "off"`), but on EC2: `sh_invoices` has 0 rows, no `[sh-cron]` log lines, and `SH_MONTH_END_CRON` isn't set in the env. Moot until S&H inventory exists (no S&H boxes onboarded yet → nothing to bill even if it fires). **Re-verify before the first S&H box goes live.** When reliability matters, consider moving it off in-process node-cron to a host-level cron / external trigger so a redeploy can't silently skip a month.
