  ...
  const authUser = (e) => {
    ...
    fetch(`api/v1/auth/${endpoint}`, {
      ...
      .then((res) => {
        if (!res.ok) {
          endpoint === 'login'
            ? setPopup('ERROR There was an error logging in, try again.')
            : setPopup('ERROR There was an error when creating your account, try again.');
        }
        if (res.status !== 200) {
          let dataHolder = {
            user: { permissions: 'unauthorized', message: 'login failed please try again' }
          };
          return dataHolder;
        }
        return res.json();
