...
function SendInvoice({generate}) {
  ...
  const updateInvoiceContainers = (e) => {
    ...
    for(var i = 0; i < numSelected; i++){
      ...
      selectedArr.push(options.find((element) => element.unit_number === currentOption))
    }
    ...
    <div className='repContainer'>
      <div className='card'>
        ...
        <div className="reportDesc">
          <p>Create an invoice and send a copy to both the customer and company with an option to print. <br/> <br/><span className='nowrap'>Select Unit(s) (Shift+Click to select multiple units):</span></p>
        </div>
        <label>
          <select multiple="true" defaultValue="" onChange={updateInvoiceContainers} className='repSelect'>
            {options.map((option) => {
              return(
        ...
        {invoiceContainers.length > 0 && <InvoiceForm containerData={invoiceContainers}/>}
      </div>
    </div>
  )
}
