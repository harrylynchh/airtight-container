...
function SoldRow({container, onDelete, outbound}) {
    ...
    const updateNotes = (e) => {
      setCurrentContainer((prev) => ({
        ...prev, invoice_notes: e.target.value,
      }))
    }
    const postNotes = () => {
    ...
    const markOutbound = () => {
      ...
      fetch(`/api/v1/inventory/outbound/${currentContainer.id}`, {
        ...
        if(!data) return
        setCurrentContainer((prev) => ({
           ...prev, outbound_date: data.data.inventory[0].outbound_date,
           state: 'outbound',
        }
        ));
      })
    ...
    return (
      <>
        <tr key={currentContainer.id}>
          ...
          <td className='dateRow'>{currentContainer.sold_date.substr(0, 10)}<br/>{outboundTime}</td>
          <td className='dateRow'>{currentContainer.outbound_date.substr(0, 10)} <br/>{invTime}</td>
          ...
          <td colSpan={3}><input type="text" value={currentContainer.invoice_notes} onChange={updateNotes} onBlur={postNotes}></input></td>
          <td><button className="tableBtn" onClick={() => setShowEdit(!showEdit)}><img src={edit} alt="Edit Unit" width="20px"/></button></td>
          <td>{<button className="tableBtn" onClick={() => markOutbound()}><img src={outboundIcon} alt="Outbound" width="20px"/></button>}</td>
          <td className='soldDeleteBtn'><button className="tableBtn deleteBtn" onClick={() => onDelete(currentContainer.id, currentContainer.inventory_id)}><img src={trash} alt="Delete" width="20px"></img></button></td>
        </tr>
        {showEdit && <tr><td colSpan={18}><UpdateSold container={currentContainer} save={saveEdits} close={closeEdit}/></td></tr>}
      </>
    )
}
