...
function Row({ container, onDelete }) {
  ...
  const saveOutbound = (container) => {
    ...
    fetch(`/api/v1/inventory/sold/${container.id}`, {
      ...
      if (!res.ok) {
        setPopup(
          'ERROR Ensure fields are properly formatted and that this unit has not already been assigned to outbounds.'
        );
      } else {
    ...
    fetch(`/api/v1/inventory/state/${currentContainer.id}`, {
      ...
      if (!res.ok) {
        ...
        newState === 'hold'
          ...
          : setPopup('Hold succesfully removed.');
      }
    });
  };
  return (
    <>
      <tr key={currentContainer.id}>
        ...
        <td className="dateRow">
          {currentContainer.date.substr(0, 10)} <br />
          {est}
        </td>
        ...
        <td>
          ...
          {currentContainer.aquisition_price ? currentContainer.aquisition_price : 'N/A'}
        </td>
        <td className="stateRow">
          <img src={checkState()} alt="whoops" width="25px" height="25px"></img>
        </td>
        <td>
          <input
            type="text"
        ...
        <td className="placeholderRow">
          <button className="tableBtn" onClick={() => setHold()}>
            <span className="btnText">HOLD</span>
          </button>
        </td>
        <td className="placeholderRow">
          <button
            className="tableBtn"
            onClick={
              currentContainer.state !== 'sold' && currentContainer.state !== 'outbound'
                ? () => setShowSold(!showSold)
                : () => setPopup('ERROR Container is already sold or offiste, can not be sold.')
            }>
            <span className="btnText">SELL</span>
          </button>
        </td>
        <td className="placeholderRow">
          <button className="tableBtn" onClick={() => goToContainerPage(currentContainer.id)}>
            <img src={view} alt="View Unit" width="20px" />
          </button>
        </td>
        <td className="placeholderRow">
          <button className="tableBtn" onClick={() => setShowForm(!showForm)}>
            <img src={edit} alt="Edit Unit" width="20px" />
          </button>
        </td>
        <td className="placeholderRow">
          <button className="tableBtn deleteBtn" onClick={() => onDelete(currentContainer.id)}>
      ...
      {showSold && (
        <tr className="updateRow">
          <td colSpan={16}>
            <OutboundForm container={currentContainer} save={saveOutbound} close={closeOutbound} />
          </td>
        </tr>
      )}
      {showForm && (
        <tr className="updateRow">
          <td colSpan={16}>
            <UpdateForm container={currentContainer} save={saveChanges} close={closeUpdate} />
          </td>
        </tr>
      )}
    </>
