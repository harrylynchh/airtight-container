...
function SoldList() {
  ...
  useEffect( () => {
    ...
    const handleDelete = async (id, inv_id) => {
      ...
      try {
          const res = await fetch(`http://localhost:8080/api/v1/inventory/sold/${id}`, {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json"
              },
      ...
      try {
        const res = await fetch(`http://localhost:8080/api/v1/inventory/sold/available/${id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
            ...
            <table className="inventoryTable">
                <thead>
                    <tr className="inventoryHeader">
                        ...
                        <th scope="col" className='displayCol' colspan={3}>
                            <label><span className="pageDisplay">Results Per Page:</span></label><br/>
                ...
                <tbody className='inventoryBody'>
                    {filters.length > 0 ? inventory.map(container => {
                        return(
                            !filters.includes(container.id) && <SoldRow container={container} onDelete={handleDelete} key={container.id} outbound={outbound}/>
                        )
                    }) : 
                    currentPage.map((container) => {
                            return(
                                <SoldRow container={container} onDelete={handleDelete} key={container.id} outbound={outbound}/>
                            )
                        })
                    }
                </tbody>
