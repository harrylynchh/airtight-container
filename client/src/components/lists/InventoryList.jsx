  ...
  useEffect( () => {
    ...
    const handleDelete = async (id) => {
      ...
      try {
          ...
          setInventory(inventory.filter(container => {
            ...
            <table className="inventoryTable">
                <thead>
                    <tr className="inventoryHeader">
                        ...
                        <th scope="col" className='damageCol'>Damage</th>
                        <th scope="col" className='tcCol'>Trucking Company</th>
                        <th scope="col">Acceptance Number</th>
                        <th scope="col" className='scCol'>Sale Company</th>
                        <th scope="col">Aquisition Price</th>
                        <th scope="col">State</th>
                        <th scope="col">Notes</th>
                        <th scope="col" className='placeholderRow pageRow' colSpan={2}>
                            <button className='arrow' onClick={page !== 1 ? () => setPage(page-1) : () => setPage(page)}>
                                <img src={leftarrow} alt="left" width='10px'></img>
                            </button>
                            <span className='pageDisplay'>Page {page} of {maxPageNumber}</span>
                            <button className='arrow' onClick={page !== maxPageNumber ? () => setPage(page + 1) : () => setPage(page)}>
                                <img src={rightarrow} alt="right" width='10px'></img>
                            </button>
                        </th>
                        <th scope="col" className='placeholderRow' colSpan={3}>
                            <label><span className="pageDisplay">Results Per Page:</span></label>
                ...
                <tbody className='inventoryBody'>
                    {filters.length > 0 ? inventory.map(container => {
                        return(
                            !filters.includes(container.id) && <Row container={container} onDelete={handleDelete} key={container.id}/>
                        )
                    }) : 
                    currentPage.map((container) => {
                            return(
                                <Row container={container} onDelete={handleDelete} key={container.id}/>
                            )
                        })
                    }
                </tbody>
