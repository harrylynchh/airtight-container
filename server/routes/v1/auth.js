...
const checkAuth = (req, res, next) => {
	if (req.session.userId) return next();
		...
		res.status(401).json({
			...
			user: { email: "unauthorized", permissions: "unauthorized" },
		});
	}
};
	...
	async (req, res) => {
		...
		try {
			...
			const results = await db.query(
				"INSERT INTO users (email, hashed_password, permissions) VALUES ($1, $2, 'none')",
				[req.body.email, hash]
			);
			res.status(200).json({
				status: "success",
...
router.post("/login", async (req, res) => {
	...
	if (!req.body.email || !req.body.password)
		return res.status(400).json({
			...
			user: { email: "invalid", permissions: "unauthorized" },
		});
