...
router.post('/', checkAdmin, async (req, res) => {
  try {
    const results = await db.query('INSERT INTO releases (company, number) VALUES ($1, $2)', [
      req.body.company,
      req.body.number
    ]);
    res.status(200).json({
...
router.put('/:id', checkAdmin, async (req, res) => {
  try {
    const results = await db.query('UPDATE releases SET company = $1, number = $2 where id=$3', [
      req.body.company,
      req.body.number,
      req.params.id
    ]);
