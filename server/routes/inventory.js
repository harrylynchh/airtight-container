...
router.get('/state', checkAuth, async (req, res) => {
  try {
    const results = await db.query('select * from inventory WHERE state=$1 ORDER BY id', [
      req.body.state
    ]);
    res.status(200).json({
      status: 'success',
...
router.put('/notes/:id', checkAdmin, async (req, res) => {
  try {
    const results = await db.query('UPDATE inventory SET notes = $1 where id = $2 returning *', [
      req.body.notes,
      req.params.id
    ]);
    res.status(200).json({
...
router.put('/state/:id', checkAdmin, async (req, res) => {
  try {
    const results = await db.query('UPDATE inventory SET state = $1 where id = $2 returning *', [
      req.body.state,
      req.params.id
    ]);
    res.status(200).json({
...
router.put('/outbound/:id', checkAdmin, async (req, res) => {
  try {
    const results = await db.query(
      'UPDATE sold SET outbound_date = CURRENT_TIMESTAMP WHERE id=$1 returning *',
      [req.params.id]
    );
    res.status(200).json({
      status: 'success',
